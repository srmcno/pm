...
function accountCard(a,rank){
  ...
  const estimated=executionEstimate(a),legacy=estimated?.legacyFeeLegs?`<p class="muted">Historical fees differ from today’s assumption. Same fills at 0.90% fees + 0.10% slippage: ${money(estimated.net)} closed net. This is a sensitivity calculation, not real profit.</p>`:'';
  return `<article class="account-card"><div class="card-head"><div class="venue"><span class="rank">#${rank}</span> ${esc(a.name)}</div><span class="pill ${a.status==='established'?'status-established':''}">${label}</span></div><div class="equity">${money(a.equity)}</div><div class="small-label">Equity · ${money(a.equity-a.initialCapital)} total P&L including open marks</div><dl class="metrics"><div><dt>Closed paper P&L</dt><dd class="${color(a.realizedPnl)}">${money(a.realizedPnl)}</dd></div><div><dt>Cash</dt><dd>${money(a.cash)}</dd></div><div><dt>Fee drag</dt><dd>${money(a.fees)}</dd></div></dl><div class="account-state"><strong>${esc(status)}</strong><p>${esc(detail)}</p></div><div class="card-bottom"><span>${a.positions.length} open · ${a.trades.length} closed</span><span>${decisions.length} market decisions</span></div>${legacy}<details class="strategy-details"><summary>Hypothesis and evidence</summary><p>${esc(strategy?.description||'Strategy definition unavailable.')}</p><p>${a.stats?.trades?`${a.stats.wins} wins, ${a.stats.losses} losses; net expectancy ${money(a.stats.expectancy)} per closed trade; profit factor ${finite(a.stats.profitFactor)?a.stats.profitFactor.toFixed(2):'not meaningful yet'}.`:'No closed-trade evidence yet. Activity is not proof of an edge.'}</p></details></article>`;
}
function renderUniverse(){
  ...
  $('universe-note').textContent=`${u.ranking||'USD notional, spread and visible depth'} · ${selected} selected · oldest fresh book ${finite(u.maxBookAgeSeconds)?u.maxBookAgeSeconds.toFixed(1)+'s':'unavailable'}`;
}
function renderStudy(){
  const study=data.rotationStudy;
  if(!study){$('rotation-study').textContent='Waiting for at least 80 fresh markets before starting the 40-versus-100 comparison. No results have been invented.';return;}
  $('rotation-study').innerHTML=`<p>Started ${when(study.startedAt)} · Each account began with $1,000; earlier profits are excluded.</p><div class="account-grid">`+Object.entries(study.accounts).map(([id,a])=>{
    const stress=executionEstimate(a,{feeRate:.012,slippageRate:.003}),audit=a.executionAudit||{},coverage=study.coverage[id];
    return `<article class="account-card"><h3>${id==='control'?'40-pair control':'100-pair expanded test'}</h3><div class="equity">${money(a.equity-1000)}</div><p>Estimated net gain including open liquidation marks${a.markComplete?'':' · incomplete / stale marks'}</p><dl class="metrics"><div><dt>Closed paper net</dt><dd>${money(a.realizedPnl)}</dd></div><div><dt>Closed / open</dt><dd>${a.trades.length} / ${a.positions.length}</dd></div><div><dt>Fresh markets</dt><dd>${coverage.ready} / ${coverage.limit}</dd></div></dl><p>Higher-cost closed net: <strong>${money(stress?.net)}</strong><br><span class="muted">Same fills at 1.20% fees + 0.30% extra slippage per side. A stress scenario, not a forecast.</span></p><p class="muted">${audit.gapCount||0} scan gaps over 15 minutes · ${audit.possibleMissedExits||0} possible missed intracycle exits. Neither exact stop fills nor continuous protection are assumed.</p></article>`;
  }).join('')+`</div><p class="muted">Base model: 0.90% taker fees + 0.10% extra slippage each side, plus observed spread and walked depth. Your account fees and asset eligibility remain unverified. Funding, withdrawal, hosting costs and taxes are excluded. Real orders: off.</p>`;
}
