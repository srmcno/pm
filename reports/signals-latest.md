# Smart-money signals

Generated 2026-08-14 15:11 UTC · window 48h · 60 qualified wallets watched · minimum 3 independent backers per signal.

Each signal marks an outcome that several qualified wallets have independently net-bought within the window. Signals are inputs for review, not trade instructions.

## 1. Counter-Strike: FUT Esports vs MOUZ (BO3) - Esports World Cup Group C — **MOUZ**

- Score **5.76** · 4 backers · net $90,856 staked
- Backers' average entry 0.58 → current price **0.15** (moved against them -0.43 since entry)
- Resolves by 2026-08-14

| backer | 90d PnL rank quality | net stake | their entry | conviction ×median |
|---|---|---|---|---|
| AV23IUa | 0.60 | $47,819 | 0.64 | 10.0× |
| GreenBerry1337 | 0.48 | $4,553 | 0.4 | 10.0× |
| CORGI8 | 0.48 | $30,519 | 0.64 | 10.0× |
| BajaBlaster | 0.40 | $7,964 | 0.6569 | 10.0× |

## 2. Counter-Strike: PARIVISION vs 100 Thieves (BO3) - Esports World Cup Group C — **100 Thieves**

- Score **3.03** · 3 backers · net $42,756 staked
- Backers' average entry 0.46 → current price **0.45** (roughly flat -0.01 since entry)
- Resolves by 2026-08-14

| backer | 90d PnL rank quality | net stake | their entry | conviction ×median |
|---|---|---|---|---|
| 0xf5592765133941c40C77e52Bc56bbe7EC167F462-1755612447740 | 0.44 | $12,324 | 0.46 | 10.0× |
| AV23IUa | 0.60 | $18,897 | 0.46 | 10.0× |
| StasPanda | 0.34 | $11,535 | 0.4501 | 10.0× |
