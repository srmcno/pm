# Smart-money signals

Generated 2026-08-26 10:00 UTC · window 48h · 59 qualified wallets watched · minimum 3 independent backers per signal.

Each signal marks an outcome that several qualified wallets have independently net-bought within the window. Signals are inputs for review, not trade instructions.

## 1. Counter-Strike: Aurora Gaming vs G2 (BO3) - BLAST Open Porto Group A — **Aurora Gaming**

- Score **4.88** · 3 backers · net $13,435 staked
- Backers' average entry 0.48 → current price **0.28** (moved against them -0.21 since entry)
- Resolves by 2026-08-26

| backer | 90d PnL rank quality | net stake | their entry | conviction ×median |
|---|---|---|---|---|
| 0x076daa87 | 0.80 | $542 | 0.48 | 10.0× |
| danielwolfmorales3pddb6dl6 | 0.43 | $4,020 | 0.49 | 10.0× |
| RegardedMoney | 0.53 | $8,873 | 0.48 | 10.0× |
