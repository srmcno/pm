...
The runtime bounds new risk to the configured $20 allocation ceiling and $5 absolute order ceiling. The default and expanded profiles allow 1% modeled loss per entry; the opt-in active profile allows 2%. The default capacity profile allows three positions, 20% maximum entry weight and 60% exposure. Venue minimums can make a small test unable to trade. The software must skip such a setup rather than increase the allocation or weaken the risk checks. Profits do not authorize a larger allocation. The $2 trading loss trigger is a control signal, not a guaranteed maximum loss; slippage, gaps, fees and unavailable markets can exceed it. Hosting is outside that trading trigger.

### Using more of the existing allocation

The account owner can select `MM_CAPACITY_PROFILE=expanded` or `MM_CAPACITY_PROFILE=active` in the worker's private environment and save/redeploy. Omitting this variable retains `standard`; unknown or empty values stop configuration rather than silently choosing a profile. Publishing new code alone does not change the currently selected profile.

| Entry capacity | Standard | Expanded | Active |
|---|---:|---:|---:|
| Simultaneous owned positions | 3 | 10 | 10 |
| Maximum entry weight | 20% | 30% | 30% |
| Maximum combined exposure | 60% | 90% | 90% |
| Absolute order reservation ceiling | $5 | $5 | $5 |
| Modeled loss per entry | 1% | 1% | 2% |

At the initial $18.85 allocation, expanded and active capacity permit up to $16.965 of combined exposure, subject to current equity, fees, the cash ledger and venue requirements. The $5 absolute order cap still takes precedence over the 30% weight. These are entry ceilings, not target purchases; risk sizing often produces much smaller orders. The active profile raises the modeled per-entry risk ceiling from $0.1885 to $0.377 at that equity. A larger position count does not require the bot to fill every slot.

All profiles use the same frozen initial allocation, actual-fee checks, native protection, signal confirmation and $2 loss trigger. Deposits and unrelated account holdings are not adopted. A profile change never resizes an existing position, modifies its immutable protection plan, resets accounting or clears a loss/recovery hold. Switching back to standard blocks additional entries when over its limits; it does not force liquidation. The selected profile and per-entry risk fraction appear in worker status after deployment. Active changes sizing only; it does not create more signals or establish that the strategy has a profitable edge.
