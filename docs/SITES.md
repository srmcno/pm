...
The prediction collector runs every ten minutes, best effort. The opportunity workflow runs every five minutes and advances the dynamic crypto tournament plus comparison-only arbitrage research. International copy entries are retired, with a separate six-hour official-settlement-only job. Tournament ledgers remain separate from crypto comparisons; comparison-only arbitrage has no ledger effect. Retired international collectors and the earlier $50 copy account remain paused. Dates, partial collections and failures remain explicit. Data-only updates do not require an interface deployment.
...
Publish the same committed source and static artifact to Pages and the existing private Site, then verify terminal deployment success and release identity. A GitHub source update is not proof that Sites has updated. Do not expose credentials in files, logs, git configuration or static output. The existing maintenance task should be reused rather than duplicated.
