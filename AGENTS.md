...
The separate `scripts/live/` Coinbase rotation worker is real-capable software with an initial disarmed deployment, documented in `docs/COINBASE-WORKER.md`. The earlier `scripts/execution/coinbase.mjs` and public setup exports remain preview-only. Preserve the private ledger, operator activation boundary and exact allocation; do not activate or submit real orders while maintaining this repository. The current deployment template limits this experiment to $18.85, with no automatic topups. Credentials and execution records never belong in public dashboard snapshots.
