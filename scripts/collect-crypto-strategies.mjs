...
try{
  const rawPrevious=await read(statePath,null),now=Date.now()/1000;
  const previous=rawPrevious?migrateCompetition(rawPrevious,now):null;if(previous)validateCompetition(previous);
  ...
  const feed=await collectCoinbaseMarkets({cached,requiredProducts,maxMarkets:40,preselect:52});
  ...
  const snapshot={...state,generatedAt:now,venue:'Coinbase Exchange public books / Coinbase Advanced U.S. fee model',policy:POLICY,feeProfile:FEE_PROFILE,strategies:STRATEGIES,
    ...
    notes:[
      'Eight independent $1,000 synthetic research bankrolls. Existing rotation/recovery ledgers were migrated without a reset.',
  ...
  if(!feed.markets.length)process.exitCode=1;
