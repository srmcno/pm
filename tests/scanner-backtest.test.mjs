...
function fixture(){
  ...
  const trigger=hours.at(-2);trigger[4]+=2;trigger[2]=trigger[4]+.1;trigger[5]=2200;
  const last=hours.at(-1);last.splice(1,5,127.8,128.7,128.45,128.6,1000);
  const value=last[4];
...
test('a replay cannot use a still-forming hourly bar as the required confirmation',()=>{
  const data=fixture();
  // The trigger ends at the replay start; its confirming bar ends one hour later.
  data.series['BTC-USD'].hours.forEach(row=>row[0]+=3600);
  const r=replay(data);assert.equal(r.trades,0);assert.equal(r.finalEquity,1000);
});
